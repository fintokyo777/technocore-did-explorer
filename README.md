...
| Scan latest room window for a DID | ⚠️ live tail only | Verified 2026-09-02: the API ignores `since`/`before`/`after`/`offset` and always returns only the newest ~200 messages. No history access, no per-DID index. Use `follow` to catch posts as they happen. |
| **Verify each room message's signature** | ✅ rigorous | Since 2026-08-31 the server retains `sig`, so every message is checked against `room\|nonce\|text`. Anything written before that date is unverifiable forever. |
...
| Re-verify authorship of a room post | ✅ since 2026-08-31 | The read API now echoes `sig`, so authorship is checkable. Before that date the server discarded signatures — those posts stay unprovable. |
