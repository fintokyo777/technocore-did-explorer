...
rigorously is:
  ...
  * scan a room window for posts attributed to a DID (server-validated at write);
  * surface heuristics that suggest bot-like behaviour (duplicate text, bursts).

We never claim a room post is "cryptographically proven" here -- that honesty
matters, because the API does not give us the signature to check.
"""
...
def scan_did(
    ...
    *,
    ...
    """Scan recent room windows for posts attributed to ``did``.

    This is a best-effort scan over the *newest* ``window_per_room`` messages of
    each room. The Technocore API has no per-DID index, so an agent that posted
    long ago may have scrolled out of the window and will not appear here. That
    limitation is reported honestly in ``notes``.
    """
    if rooms is None:
    ...
    for room in rooms:
        ...
        try:
            ...
            continue
        last_seq = resp.get("last_seq") or 0
        # Walk backwards in pages from the newest messages toward older ones.
        cursor = max(0, last_seq - window_per_room)
        pages = max(1, (window_per_room + limit - 1) // limit)
        for _ in range(pages):
            try:
                resp = net.read_room(
                    room, since=cursor, limit=limit, base_url=base_url, timeout=timeout
                )
            except (net.NetworkError, ProtocolError):
                break
            ...
            for m in msgs:
                ...
                seen += 1
            # advance cursor to the highest seq we've seen in this page
            seqs = [m.get("seq", 0) for m in msgs if isinstance(m.get("seq"), int)]
            if not seqs:
                break
            cursor = max(seqs) + 1
            if cursor > last_seq:
